use crate::{AggPrimitive, Args, Color, GrayArgs, ImageAccessorGray, Order, SpanGenerator};

// NOT TESTED
//=======================================================SpanPatternGray
pub struct SpanPatternGray<S: ImageAccessorGray> {
    src: S,
    offset_x: u32,
    offset_y: u32,
    alpha: u8,
...
impl<S: ImageAccessorGray> SpanPatternGray<S> {
...
impl<S: ImageAccessorGray> SpanGenerator for SpanPatternGray<S> {
    type C = S::ColorType;
    ...
    fn generate(&mut self, span: &mut [S::ColorType], x: i32, y: i32, len: u32) {
        ...
        for i in 0..len {
            unsafe {
                *span[i as usize].v_mut() = *p.offset(S::OrderType::R as isize);
            }
