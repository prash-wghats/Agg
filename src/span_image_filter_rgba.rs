...
use crate::{AggPrimitive, Args, Color, ImageAccessorRgb, Order, PixFmt, RgbArgs, SpanGenerator};
...
pub struct SpanImageFilterRgbaNn<'a, S: ImageAccessorRgb, I: Interpolator> {
    base: SpanImageFilter<'a, S, I>,
}

impl<'a, S: ImageAccessorRgb, I: Interpolator> SpanImageFilterRgbaNn<'a, S, I> {
...
impl<'a, S: ImageAccessorRgb, I: Interpolator> SpanGenerator for SpanImageFilterRgbaNn<'a, S, I> {
    type C = S::ColorType;
    fn generate(&mut self, span: &mut [S::ColorType], x: i32, y: i32, len: u32) {
        ...
        for i in 0..len {
            ...
            let fg_ptr = self
                ...
                .source
                .span(
                    x >> ImageSubpixelScale::Shift as i32,
                    y >> ImageSubpixelScale::Shift as i32,
                    1,
                )
                .as_ptr() as *const u8
            ...
            unsafe {
                ...
                *span[i as usize].a_mut() = *fg_ptr.offset(S::OrderType::A as isize);
            }
...
pub struct SpanImageFilterRgbaBilinear<'a, S: ImageAccessorRgb, I: Interpolator> {
    base: SpanImageFilter<'a, S, I>,
}

impl<'a, S: ImageAccessorRgb, I: Interpolator> SpanImageFilterRgbaBilinear<'a, S, I> {
    pub const BASE_SHIFT: u32 = S::ColorType::BASE_SHIFT;
    pub const BASE_MASK: u32 = S::ColorType::BASE_MASK;
...
impl<'a, S: ImageAccessorRgb, I: Interpolator> SpanGenerator
    for SpanImageFilterRgbaBilinear<'a, S, I>
{
    type C = S::ColorType;
    fn generate(&mut self, span: &mut [S::ColorType], x: i32, y: i32, len: u32) {
        self.base.interpolator.begin(
        ...
        for i in 0..len {
            ...
            unsafe {
                ...
                fg[3] += weight * (*fg_ptr.offset(3)).into_u32();
            }
            ...
            *span[i as usize].r_mut() =
                from_u32!(fg[S::OrderType::R as usize] >> (ImageSubpixelScale::Shift as i32 * 2));
            *span[i as usize].g_mut() =
                from_u32!(fg[S::OrderType::G as usize] >> (ImageSubpixelScale::Shift as i32 * 2));
            *span[i as usize].b_mut() =
                from_u32!(fg[S::OrderType::B as usize] >> (ImageSubpixelScale::Shift as i32 * 2));
            *span[i as usize].a_mut() =
                from_u32!(fg[S::OrderType::A as usize] >> (ImageSubpixelScale::Shift as i32 * 2));
...
pub struct SpanImageFilterRgbaBilinearClip<
    'a,
    C: Color + RgbArgs,
    S: PixFmt<C = C>,
    I: Interpolator,
...
impl<'a, C: Color + RgbArgs, S: PixFmt<C = C>, I: Interpolator>
    SpanImageFilterRgbaBilinearClip<'a, C, S, I>
{
...
impl<'a, C: Color + RgbArgs, S: PixFmt<C = C>, I: Interpolator> SpanGenerator
    for SpanImageFilterRgbaBilinearClip<'a, C, S, I>
{
    type C = C;
    fn generate(&mut self, span: &mut [S::C], x: i32, y: i32, len: u32) {
        ...
        for i in 0..len {
            ...
            let mut x_lr = x_hr >> ImageSubpixelScale::Shift as i32;
            let mut y_lr = y_hr >> ImageSubpixelScale::Shift as i32;
            ...
            if x_lr >= 0 && y_lr >= 0 && x_lr < maxx && y_lr < maxy {
                ...
                fg[3] += weight * fg_ptr[off + 3].into_u32();

                weight = (x_hr * (ImageSubpixelScale::Scale as i32 - y_hr)) as u32;
                fg[0] += weight * fg_ptr[off + 4].into_u32();
                fg[1] += weight * fg_ptr[off + 5].into_u32();
                fg[2] += weight * fg_ptr[off + 6].into_u32();
                fg[3] += weight * fg_ptr[off + 7].into_u32();
                ...
                fg[0] >>= ImageSubpixelScale::Shift as i32 * 2;
                fg[1] >>= ImageSubpixelScale::Shift as i32 * 2;
                fg[2] >>= ImageSubpixelScale::Shift as i32 * 2;
                fg[3] >>= ImageSubpixelScale::Shift as i32 * 2;
            } else {
                if x_lr < -1 || y_lr < -1 || x_lr > maxx || y_lr > maxy {
                    ...
                    fg[S::O::A as usize] = back_a.into_u32();
                } else {
                    fg[0] = (ImageSubpixelScale::Scale as i32 * ImageSubpixelScale::Scale as i32)
                        as u32
                        / 2;
                    ...
                    if x_lr >= 0 && y_lr >= 0 && x_lr <= maxx && y_lr <= maxy {
                        ...
                        fg[S::O::A as usize] += back_a.into_u32() * weight;
                    }
            ...
            *span[i as usize].a_mut() = from_u32!(fg[S::O::A as usize]);
...
//==============================================SpanImageFilterRgba2x2
pub struct SpanImageFilterRgba2x2<'a, S: ImageAccessorRgb, I: Interpolator> {
    base: SpanImageFilter<'a, S, I>,
}

impl<'a, S: ImageAccessorRgb, I: Interpolator> SpanImageFilterRgba2x2<'a, S, I> {
    pub const BASE_SHIFT: u32 = S::ColorType::BASE_SHIFT;
    pub const BASE_MASK: u32 = S::ColorType::BASE_MASK;
...
impl<'a, S: ImageAccessorRgb, I: Interpolator> SpanGenerator for SpanImageFilterRgba2x2<'a, S, I> {
    type C = S::ColorType;
    fn generate(&mut self, span: &mut [S::ColorType], x: i32, y: i32, len: u32) {
        let mut fg: [u32; 4] = [0; 4];
        ...
        for i in 0..len {
            ...
            let mut weight: u32;
            fg[0] = ImageFilterScale::Scale as i32 as u32 / 2;
            fg[1] = fg[0];
            fg[2] = fg[0];
            fg[3] = fg[0];
            ...
            fg[3] >>= ImageFilterScale::Shift as i32;

            if fg[S::OrderType::A] > Self::BASE_MASK {
                fg[S::OrderType::A] = Self::BASE_MASK;
            }
            if fg[S::OrderType::R] > fg[S::OrderType::A] {
                fg[S::OrderType::R] = fg[S::OrderType::A];
            }
            if fg[S::OrderType::G] > fg[S::OrderType::A] {
                fg[S::OrderType::G] = fg[S::OrderType::A];
            }
            if fg[S::OrderType::B] > fg[S::OrderType::A] {
                fg[S::OrderType::B] = fg[S::OrderType::A];
            }
            ...
            *span[i as usize].a_mut() = from_u32!(fg[S::OrderType::A as usize]);
...
//==================================================SpanImageFilterRgba
pub struct SpanImageFilterRgba<'a, S: ImageAccessorRgb, I: Interpolator> {
    base: SpanImageFilter<'a, S, I>,
}

impl<'a, S: ImageAccessorRgb, I: Interpolator> SpanImageFilterRgba<'a, S, I> {
    pub const BASE_SHIFT: u32 = S::ColorType::BASE_SHIFT;
    pub const BASE_MASK: u32 = S::ColorType::BASE_MASK;
...
impl<'a, S: ImageAccessorRgb, I: Interpolator> SpanGenerator for SpanImageFilterRgba<'a, S, I> {
    type C = S::ColorType;
    fn generate(&mut self, span: &mut [S::ColorType], x: i32, y: i32, len: u32) {
        let (mut x, mut y) = (x, y);
        self.base.interpolator.begin(
        ...
        for i in 0..len {
            ...
            loop {
                ...
                loop {
                    ...
                    unsafe {
                        ...
                        fg[3] += weight * (*fg_ptr.offset(3)).into_i32();
                    }
            ...
            if fg[S::OrderType::A] > Self::BASE_MASK as i32 {
                fg[S::OrderType::A] = Self::BASE_MASK as i32;
            }
            if fg[S::OrderType::R] > fg[S::OrderType::A] {
                fg[S::OrderType::R] = fg[S::OrderType::A];
            }
            ...
            *span[i as usize].a_mut() = from_i32!(fg[S::OrderType::A as usize]);
...
pub struct SpanImageResampleRgbaAffine<'a, S: ImageAccessorRgb> {
    base: SpanImageResampleAffine<'a, S>,
}

impl<'a, S: ImageAccessorRgb> SpanImageResampleRgbaAffine<'a, S> {
    pub const BASE_SHIFT: u32 = S::ColorType::BASE_SHIFT;
    pub const BASE_MASK: u32 = S::ColorType::BASE_MASK;
...
impl<'a, S: ImageAccessorRgb> SpanGenerator for SpanImageResampleRgbaAffine<'a, S> {
    type C = S::ColorType;
    fn generate(&mut self, span: &mut [S::ColorType], x: i32, y: i32, len: u32) {
        let (mut x, mut y) = (x, y);
...
pub struct SpanImageResampleRgba<'a, S: ImageAccessorRgb, I: Interpolator> {
    base: SpanImageResample<'a, S, I>,
}

impl<'a, S: ImageAccessorRgb, I: Interpolator> SpanImageResampleRgba<'a, S, I> {
    pub const BASE_SHIFT: u32 = S::ColorType::BASE_SHIFT;
    pub const BASE_MASK: u32 = S::ColorType::BASE_MASK;
    const DOWNSCALE_SHIFT: u32 = ImageFilterScale::Shift as i32 as u32;
    pub fn new(src: &'a mut S, inter: &'a mut I, filter: ImageFilterLut) -> Self {
...
impl<'a, S: ImageAccessorRgb, I: Interpolator> SpanGenerator for SpanImageResampleRgba<'a, S, I> {
    type C = S::ColorType;
    fn generate(&mut self, span: &mut [S::ColorType], x: i32, y: i32, len: u32) {
        let diameter = self.base.filter.diameter() as i32;
        let filter_scale = diameter << ImageSubpixelScale::Shift as i32;
